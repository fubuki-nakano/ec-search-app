...
def index():
    ...
    if searched:
        ...
        if sort not in SORTS:
            ...
            # ここまでの指定されたものをAPIに渡す
        if not errors:
            products, errors = search_products(keyword, selected, int(limit), sort)
            # APIから帰ってきた情報をHTMLへ渡す
    return render_template(
        "index.html", sites=SITES, limits=LIMITS, sorts=SORTS,
        keyword=keyword, selected=selected, limit=limit, sort=sort,
        products=products, errors=errors, searched=searched,
    )
